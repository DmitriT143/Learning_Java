...
public class Task5 {
    ...
    //Exceptions handling are as follows: Is A/B->H/G; lower radian to upper radian or just go by radius if degree < 91
    //Lastly if Degree > 91 go by center
    //degrees would be counted as stated above
    public static String SpiderPath(String spiderPos, String flyPos){
        String[] curFlyPos = flyPos.split("");
        String[] curSpiderPos = spiderPos.split("");
        String path = spiderPos;
        String exceptionSpider= "AB";
        String exceptionFly = "GH";
        int diffBetweenRad = Math.abs((int) curFlyPos[0].charAt(0) - (int) curSpiderPos[0].charAt(0));
        int diffBetweenRadius = (int) curFlyPos[1].charAt(0) - (int) curSpiderPos[1].charAt(0);
        if(diffBetweenRad > 5 && exceptionSpider.contains(curSpiderPos[0]) && exceptionFly.contains(curFlyPos[0]))
        {
            while (diffBetweenRadius < 0){
                curSpiderPos[1] = String.valueOf(Integer.parseInt(curSpiderPos[1]) - 1);
                diffBetweenRadius++;
                path = path + ","+String.join("",curSpiderPos);
            }
            while (!curSpiderPos[0].equals("A")){
                curSpiderPos[0] = Character.toString((int) curSpiderPos[0].charAt(0)-1);
                path = path + ","+String.join("",curSpiderPos);
            }
            curSpiderPos[0] = "H";
            diffBetweenRad = Math.abs((int) curFlyPos[0].charAt(0) - (int) curSpiderPos[0].charAt(0));
            path = path + ","+String.join("",curSpiderPos);
        }
        if(diffBetweenRad <= 2){
            while (diffBetweenRadius < 0){
            ...
            while (diffBetweenRad != 0){
                curSpiderPos[0] = ((int) curFlyPos[0].charAt(0) - (int) curSpiderPos[0].charAt(0)>0)? Character.toString(curSpiderPos[0].charAt(0)+1):Character.toString(curSpiderPos[0].charAt(0)-1);
                diffBetweenRad = Math.abs((int) curFlyPos[0].charAt(0) - (int) curSpiderPos[0].charAt(0));
                path = path + ","+String.join("",curSpiderPos);
            }
            while (diffBetweenRadius > 0){
            ...
            for (int i = 0; i < Integer.parseInt(spiderPos.substring(1)); i++) {
                curSpiderPos[1] = String.valueOf(Integer.parseInt(curSpiderPos[1])-1);
                path = path + ","+String.join("",curSpiderPos);
            }
            curSpiderPos[0] = curFlyPos[0];
            for (int i = 0; i < Integer.parseInt(flyPos.substring(1)); i++) {
                curSpiderPos[1] = String.valueOf(Integer.parseInt(curSpiderPos[1])+1);
                path = path + "," + String.join("",curSpiderPos);
            }
        }
